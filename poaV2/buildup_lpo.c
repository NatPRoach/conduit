...
LPOReturnResult_T build_lpo_from_fasta(FILE *seq_ifile,
						 ...
						 (int,int,LPOLetter_T [],LPOLetter_T [],ResidueScoreMatrix_T *))
  {
    ...
    FILE *errfile=stderr;
    int exit_code=0;
    LPOSequence_T *seq=NULL,*lpo_out=NULL;
    char *comment=NULL,*subset_file=NULL,*seq_filename=NULL,*pair_score_file=NULL;
    LPOSequence_T **input_seqs=NULL;
    ResidueScoreMatrix_T *score_matrix;
    CALLOC(score_matrix, 1, ResidueScoreMatrix_T)
    ...
    if (!is_silent)
        fprintf(errfile,"...Read %d sequences...\n",nseq);
    for (i=0; i<nseq; i++) {
    ...
    if (n_input_seqs == 0) { /* HMM.. NO DATA. */
      ...
      lpo_out =  buildup_progressive_lpo(n_input_seqs,
                                     input_seqs,
                                     score_matrix,
				                             0,
                                     0,
...
void free_residue_matrix(ResidueScoreMatrix_T *lpo_matrix){
  FREE(lpo_matrix->gap_penalty_x)
  FREE(lpo_matrix->gap_penalty_y)
  FREE(lpo_matrix)
}

void free_return_result(LPOReturnResult_T lpo_result){
  int i,j =0;
  for (i=0;i<lpo_result.n_input_seqs;i++) {
    for (j=0;j<lpo_result.nseq;j++) {
      if (lpo_result.input_seqs[i]==&(lpo_result.sequence[j]))
        break;
    }
    free_lpo_sequence(lpo_result.input_seqs[i],(j==lpo_result.nseq));
  }
  FREE (lpo_result.input_seqs);
  if (lpo_result.nseq>0) FREE (lpo_result.sequence);
  free_residue_matrix(lpo_result.matrix);
}