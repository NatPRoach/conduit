...
proc collapseLinearStretches(po2: TrimmedPOGraph): TrimmedPOGraph =
  ## Collapses streches of a partial order graph that do not branch at all, and
  ## returns a new version of the partial order graph with these branches collapsed
  var po = po2
...
proc jsOutput(po: TrimmedPOGraph,
  ...
  ## Outputs the data for a partial order graph in JSON format and returns the
  ## lines to write to an HTML file to generate a visualization of the partial
  ## order graph
  ##
  ## Args:
  ##  po: The partial order graph to visualize
  ##  outfile: The file to write HTML output to
  ##  highlight_path1: the indices of the first path to highlight, if any
  ##  highlight_path2: the indices of the second path to highlight, if any
  ##  collapse: whether to collapse the linear stretches of the partial order
...
proc htmlOutput*(po: TrimmedPOGraph,
                 ...
                 highlight_path1, highlight_path2: seq[uint32] = @[],
                 collapse: bool = false) =
  ## Produces an HTML file for the purposes of visualizing a provided partial
  ## order graph
  ##
  ## Args:
  ##  po: The partial order graph to visualize
