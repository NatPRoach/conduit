...
proc conduitClusterVersion(): string =
  ## Returns the version of the CONDUIT clustering tool
  return &"CONDUIT Clustering Version {package_version.ConduitVersionString} by Nathan Roac" &
...
proc summaryFromBamRecord(record: Record,
                          stranded: bool = false): AlignmentSummary =
  ## Takes a Bam Record from the nim hts lib and returns an AlignmentSummary
  ...
  var strand: char
  var spliceSites: seq[SpliceSite]
  ...
  for op in cigar:
    case int(op.op):
      ...
      of 3:
        ...
        refPosition += uint64(op.len)
        spliceSites.add(SpliceSite(start_site: start,
                                   end_site: refPosition))
      else:
        discard
  let alignmentEnd = refPosition
  result = AlignmentSummary(strand: strand,
